...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if PM_WARNS[str(chat.id)] >= MAX_FLOOD_IN_PMS:
        ...
        if custompmblock is not None:
            ...
            USER_BOT_WARN_ZERO = f"داری اسپم میکنی متاسفانه بلاک میشی"
...
async def do_pm_options_action(event, chat):
    ...
    try:
        ...
        PMMESSAGE_CACHE = {}
    if str(chat.id) not in PM_WARNS:
        text = "لطفا یکی از گزینه های بالا رو انتخاب کنید (این اخطار آخر شماست)"
...
async def do_pm_enquire_action(event, chat):
    ...
    try:
        ...
        PMMESSAGE_CACHE = {}
    if str(chat.id) not in PM_WARNS:
        text = "لطفا صبر کن تا مزدک آنلاین بشه هنوز پیامت رو سین نزده"
        await event.reply(text)
        PM_WARNS[str(chat.id)] = 1
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
async def on_plug_in_callback_query_handler(event):
    ...
    text = f"""خب الان به دکمه ها دسترسی داری

**یکی از دکمه های زیر رو انتخاب کن ⬇️ **"""
    buttons = [
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        ...
        return await event.answer(text, cache_time=0, alert=True)
    text = "اوکیه عزیزم به مزدک اطلاع دادم و وقتی آنلاین شد بهت پیام میده"
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        ...
        return await event.answer(text, cache_time=0, alert=True)
    text = "چه عالی مشکلی نیست عزیزم هرموقع مزدک آنلاین شد میگم بهت پیام بده 💚"
